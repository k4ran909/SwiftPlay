...
#define ID_TRAY_RESTART_SWIFTPLAY         3004
...
int main()
{
    ...
    hMutex = CreateMutex(NULL, TRUE, TEXT("SwiftPlayMutex"));
    if (GetLastError() == ERROR_ALREADY_EXISTS)
    {
        MessageBox(NULL, TEXT("SwiftPlay is already running!"), TEXT("SwiftPlay"), MB_ICONINFORMATION | MB_OK);
    ...
    wc.lpszClassName = TEXT("SwiftPlayClass");
...
void InitNotifyIconData(HWND hwnd)
{
    ...
    lstrcpy(nid.szTip, TEXT("SwiftPlay"));
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_TRAYICON:
        if (lParam == WM_RBUTTONDOWN)
        {
            ...
            AppendMenu(hMenu, MF_STRING | (isLocked ? MF_CHECKED : MF_UNCHECKED), ID_TRAY_LOCK_FUNCTION, TEXT("Disable SwiftPlay"));
            AppendMenu(hMenu, MF_STRING, ID_TRAY_RESTART_SWIFTPLAY, TEXT("Restart SwiftPlay"));
            AppendMenu(hMenu, MF_SEPARATOR, 0, NULL);
            AppendMenu(hMenu, MF_STRING, ID_TRAY_VERSION_INFO, TEXT("Version Info"));
            AppendMenu(hMenu, MF_STRING, ID_TRAY_EXIT_CONTEXT_MENU_ITEM, TEXT("Exit SwiftPlay"));
    ...
    case WM_COMMAND:
        ...
        case ID_TRAY_REBIND_KEYS:
            ...
            break;
        case ID_TRAY_RESTART_SWIFTPLAY:
            {
...
// Function to copy swiftplay.backup (meta folder) to the main directory
void RestoreConfigFromBackup(const std::string& backupFilename, const std::string& destinationFilename)
{
    ...
    if (CopyFile(sourcePath.c_str(), destinationPath.c_str(), FALSE)) {
        // Copy successful
        MessageBox(NULL, TEXT("Default config restored from backup successfully."), TEXT("SwiftPlay"), MB_ICONINFORMATION | MB_OK);
    } else {
        // backup.swiftplay copy failed
        DWORD error = GetLastError();
        std::string errorMsg = "Failed to restore config from backup.";
        MessageBox(NULL, errorMsg.c_str(), TEXT("SwiftPlay Error"), MB_ICONERROR | MB_OK);
    }
}

// Restore config.cfg from backup.swiftplay
void CreateDefaultConfig(const std::string& filename)
{
    std::string backupFilename = "backup.swiftplay";
...
bool LoadConfig(const std::string& filename)
{
    ...
    if (!configFile.is_open()) {
        CreateDefaultConfig(filename);  // Restore config from backup.swiftplay if file doesn't exist
        return false;
    }
    ...
    while (getline(configFile, line)) {
        ...
        else if (getline(iss, key, '=') && (iss >> value))
        {
            if (key.find("key") != string::npos)
            {
                ...
                {
                    MessageBox(NULL, TEXT("The config file contains duplicate keys across various groups. Please review and correct the setup."), TEXT("SwiftPlay Error"), MB_ICONEXCLAMATION | MB_OK);
                    MessageBox(NULL, TEXT("For more information, please contact me on kkara9009@gmail.com"), TEXT("SwiftPlay Error"), MB_ICONINFORMATION | MB_OK);
                    return false;
                }
            }
        }
